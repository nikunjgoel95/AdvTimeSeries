...
$x_{t}$ process is not stationary as variance is dependent of time.
...
\end{aligned}
$$
From the above the mean of $x_t$ is constant and variance is not dependent on time so the process is stationary.
...
In ARMA(1,1) both acf and pacf tails off
In ARMA(1,0) acf tails off and pacf cutf off after lag p (p=1 in this case)
In ARMA(0,1) acf cuts off after lag q(q=1 in this case) and pacf tails off.

### Part b)

```{r}
#acf2(arima.sim(list(order=c(1,0,1), ar=.6, ma=.9), n=100))
acf2(arima.sim(model = list(ar=.6, ma=.9), n=100))
acf2(arima.sim(model = list(ar=.6), n=100))
acf2(arima.sim(model = list(ma=.9), n=100))
#acf2(arima.sim(list(order=c(0,0,1), ar=.6, ma=.9), n=100))
```
...
Differencing seems reasonable in this case as it produces fairly stationary time series.
...
AR(1) is apprpriate for $x_{t}$ because ACF is tailing off.PACF cuts off after lag = 1 and values are nearly closely to zero.
...
Residuals do not follow any pattern.
...
** write the equation of forecasting here**

### Part g)

```{r}
sarima.for(diff(cmort), n.ahead=1,p=1,d=0,q=0)
```
